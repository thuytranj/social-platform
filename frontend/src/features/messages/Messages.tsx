...
const ConversationList = ({
  ...
  const getConvName = (conv: Conversation) => {
    ...
    return (
      otherMember?.user.profile?.full_name ||
  ...
  return (
    <div
      className={`w-full md:w-80 lg:w-96 flex flex-col border-r border-gray-200 dark:border-white/10 bg-white dark:bg-surface-50 ${activeConvId ? 'hidden md:flex' : 'flex'}`}
    >
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isLoading ? (
          ...
          <div className="text-center p-8 text-gray-600 dark:text-ink-muted">
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-surface-50">
      ...
      <div className="h-16 px-4 border-b border-gray-200 dark:border-white/10 flex items-center gap-3 bg-white/80 dark:bg-surface-50/80 backdrop-blur-md z-10 shrink-0">
        <button
          onClick={onBack}
          className="md:hidden p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-surface-200 text-gray-900 dark:text-ink"
        >
          &larr;
        </button>
        <div className="font-semibold text-ink text-lg truncate">
          {getConvName()}
        </div>
      </div>
        ...
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {!isMine && (
                <Avatar
                  src={msg.sender.profile?.avatar_url}
      ...
      <div className="p-4 bg-white dark:bg-surface-50 border-t border-gray-200 dark:border-white/10 shrink-0">
        ...
        >
          <button
            type="button"
            className="p-2 text-ink-muted hover:text-primary-500 transition-colors"
          >
            <ImageIcon size={22} />
          </button>
          <input
