  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-ink">Notifications</h1>
        {unreadCount > 0 && (
          <button 
      ...
      <div className="space-y-2">
        {notifications.length === 0 ? (
          <div className="text-center py-20 bg-gray-100 dark:bg-surface-200 rounded-2xl border border-gray-200 dark:border-white/10 border-dashed">
            <Bell size={48} className="mx-auto mb-4 text-gray-400 dark:text-ink-muted" />
            <p className="text-gray-600 dark:text-ink-muted">You're all caught up!</p>
          </div>
        ) : (
...
const NotificationItem = ({ notification, onRead }: { notification: Notification, onRead: () => void }) => {
  ...
  const getLink = () => {
    if (notification.target_id && notification.type.startsWith('post')) return `/post/${notification.target_id}`;
    if (notification.target_id && notification.type.startsWith('friend')) return `/profile/${notification.actor?.id}`;
    if (notification.group_id) return `/groups/${notification.group_id}`;
    if (notification.conversation_id) return `/messages?conv=${notification.conversation_id}`;
    return '#';
  };
  ...
  return (
    <Link 
      to={getLink()} 
      onClick={() => { if (!notification.is_read) onRead(); }}
    ...
    >
      ...
      <div className={`flex items-start gap-4 ${!notification.is_read ? 'pl-4' : ''}`}>
        <div className="relative shrink-0">
          <Avatar src={notification.actor?.profile?.avatar_url} alt={notification.actor?.username || 'User'} size="lg" />
          <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center border-2 border-white dark:border-surface-50 ${getIconBg()}`}>
            {getIcon()}
          </div>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[15px] text-gray-900 dark:text-ink leading-snug">
            <span className="font-semibold hover:underline">
              {notification.actor?.profile?.full_name || notification.actor?.username}
            </span>{' '}
