  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen sticky top-0 border-r border-gray-200 dark:border-white/10 bg-white dark:bg-surface-50 overflow-y-auto px-4 py-6 z-40">
      <Link to="/" className="flex items-center gap-2 px-2 mb-8 hover-lift">
        ...
        <span className="font-display font-bold text-xl tracking-tight text-gray-900 dark:text-ink">VibeConnect</span>
      </Link>
        ...
        <div className="mt-auto pt-4 border-t border-gray-200 dark:border-white/10">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-600 dark:text-ink-muted truncate">@{user.username}</p>
            </div>
            <Icons.ChevronRight size={18} className="text-gray-400 dark:text-ink-faint group-hover:text-gray-600 dark:group-hover:text-ink-muted" />
          </Link>
        </div>
      )}
    </aside>
