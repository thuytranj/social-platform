...
export const QK = {
  ...
  POST_REACTIONS:   (id: string) => ['post-reactions', id] as const,
  COMMENT_REPLIES:  (postId: string, commentId: string) => ['comment-replies', postId, commentId] as const,
...
export const PRIVACY_CONFIG: Record<PostPrivacy, { label: string; icon: string }> = {
  [PostPrivacy.PUBLIC]:       { label: 'Public',       icon: '🌍' },
  [PostPrivacy.FRIENDS_ONLY]: { label: 'Friends only', icon: '👫' },
  [PostPrivacy.PRIVATE]:      { label: 'Private',      icon: '🔒' },
};
